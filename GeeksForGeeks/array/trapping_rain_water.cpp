...
int32_t main()
{
    ...
    /* water capacity is equal to the 
     * minimum of left greater of i and  right greater of i minus arr[i
   */    
    
   TCase{
       accept_arr(n,arr);
       int count=0;
      int leftg[n],rightg[n];
      int left=arr[0];
       leftg[0]=arr[0];
        for (int j = 1; j <n ; ++j) {
           if(arr[j] > left){
        ...
        for (int l = 0; l < n-1; ++l) {
            count += min(leftg[l],rightg[l])-arr[l];
        }
     //   For(i,n) cout<<leftg[i]<<" ";
     //   For(i,n) cout<<rightg[i]<<" ";
        cout<<count<<endl;
   };
    return 0;
}
